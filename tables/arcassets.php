...
class TableArcAssets extends JTableNested
{
	...
	public function addMyInfo($inStr)
	{
        ...
        $query->insert($db->quoteName('#__arc_my_info'))
	...
	public function deleteMyInfo($idStr)
	{
        ...
        $query = $db->getQuery(true);
        $query->delete($db->quoteName('#__arc_my_info'));
	...
	public function editMyInfo($inStr)
	{
		$arc_input = json_decode($inStr);
		...
		if($compare_str == $arc_input->data_str)
		{
			...
			$query->update($db->quoteName('#__arc_my_info'))->set($fields )->where($conditions);
			$db->setQuery($query);
			$results = $db->execute();
